...
inductive Term2' (rep :  Type) : Type
  | var   : rep → Term2' rep
  | lam   : (rep → Term2' rep) → Term2' rep
  | app   : Term2' rep → Term2' rep  → Term2' rep

@[reducible]
def Term2 := (rep : Type) → Term2' rep

def omega : Term2 := fun _rep => .lam fun x => .app (.var x) (.var x)
def omega2 : Term2 := fun _rep => .app (omega _rep) (omega _rep)

def Term2.step2 (t : Term2) : Term2 :=
  fun rep =>
    match t rep with
      | .lam f => .lam f
      | .app (.lam f) x =>  sorry
      | .app f x => .app sorry sorry
      | .var x => .var x

def Term2.step {rep : Type} (t : Term2' rep) : Term2' rep := --sorry
    match t with
    | .lam f => .lam f
    | .app (.lam f) a => f sorry
    | .app f a => .app (Term2.step f) (Term2.step a)
    | .var x => .var x

def Term2.step3 (t : Term2' Nat) : Term2' Nat := --sorry
    match t with
    | .lam f => .lam f
    | .app (.lam f) a => f sorry
    | .app f a => .app (Term2.step f) (Term2.step a)
...
def Term2'.squash {rep : Type} : Term2' (Term2' rep) → Term2' rep
  | .var t    => t -- This is the key: replace the variable with the term it represents.
...
-/
#guard_msgs in
def usubst (e : {rep : Type} → rep → Term2' rep) (e' : Term2) : Term2 :=
  Term2'.squash (e e')
...
`"x_" ++ toString i`.
-/
def pretty {ty : Ty} (e : Term' (fun _ => String) ty) (i : Nat := 1) : String :=
  match e with
  | .var s     => s
  | .const n   => toString n
...
/-- info: "((((fun x_1 => (fun x_2 => (x_1 + x_2))) 1) 2) + 5)" -/
#guard_msgs in #eval pretty <| subst (fun x => .plus (.var x) (.const 5)) three_the_hard_way
